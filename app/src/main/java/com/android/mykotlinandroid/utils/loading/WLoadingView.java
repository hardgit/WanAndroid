...
public class WLoadingView extends View {
    ...
    public WLoadingView(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.WLoadingStyle);
    }

    public WLoadingView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.WLoadingView, defStyleAttr, 0);
        mSize = array.getDimensionPixelSize(R.styleable.WLoadingView_libx_loading_view_size, 64);
        mPaintColor = array.getInt(R.styleable.WLoadingView_android_color, Color.WHITE);
        array.recycle();
        initPaint();
    }

    public WLoadingView(Context context, int size, int color) {
        super(context);
        mSize = size;
        mPaintColor = color;
        initPaint();
